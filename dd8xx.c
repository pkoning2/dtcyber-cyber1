...
static void dd8xxInit(u8 eqNo, u8 unitNo, u8 channelNo, char *deviceName, DiskSize *size)
    ...
    */
    printf("DD844 with %d cylinders initialised on channel %o unit %o\n",
        dp->cylinder, channelNo, unitNo);
    }
...
static i32 dd8xxSeekNextSector(DiskParam *dp)
    ...
    if (dp->interlace == 1)
        ...
        if (dp->sector == dp->size.maxSectors)
            ...
            if (dp->track == dp->size.maxTracks)
                {
                logError(LogErrorLocation, "ch %o, sector wrap invalid - advance to begin of cylinder\n", activeChannel->id);
                dp->track = 0;
                dp->sector = 0;
                }
            }
