/*
** This code is under development.  Status is as follows:
...
Incomplete: unicode copy is done in classic mode, except for accented
character handling.  Unicode paste is not there yet.  Ascii terminal
copy is not working right.
...
// Map from regular characters to combining diacriticals.  This is
// used when an autobackspaced character is seen.  The map consists
// of pairs, the regular character and the corresponding combining character.
static const wxChar combinemap[] = 
    wxT ("`\u0300"          // grave
         "\u00b4\u0301"     // acute
         "^\u0302"          // circumflex
         "\u02DC\u0303"     // tilde
...
#define CWS_TERMRESTORE 2001
    ...
    // Text-copy support
    void SaveChar (int x, int y, wxChar c, bool large_p);
      ...
      m_regionWidth (0)
{
    ...
    // Status bar always exists, but we may or may not display it
    BuildStatusBar (true);
...
void PtermFrame::BuildViewMenu (int port)
{
    menuView = new wxMenu;
#if !defined (__WXMAC__)
    menuView->AppendCheckItem (Pterm_ToggleMenuBarView,
                               _("Display menu bar"), _("Display menu bar"));
    menuView->Check (Pterm_ToggleMenuBarView, ptermApp->m_showMenuBar);
#endif
    if (port > 0)
    {
        menuView->AppendCheckItem (Pterm_ToggleStatusBarView,
...
void PtermFrame::BuildPopupMenu (int port)
{
    ...
    menuPopup->AppendSeparator ();
#if !defined (__WXMAC__)
    menuPopup->AppendCheckItem (Pterm_ToggleMenuBar, _("Display menu bar"),
                                _("Display menu bar"));
    menuPopup->Check (Pterm_ToggleMenuBar, ptermApp->m_showMenuBar);
#endif
    if (port > 0)
    {
        menuPopup->AppendCheckItem (Pterm_ToggleStatusBar,
                                    _("Display status bar"),
                                    _("Display status bar"));
...
void PtermFrame::OnPasteTimer (wxTimerEvent &)
{
    ...
    if (m_pastePrint)
    {
        while (m_pasteIndex < (int) m_pasteText.Len ())
        {
            ...
            if (p != -1)
            {
                ...
                // character (like $), or an unshifted character whose shifted
                // form corresponds to a different printable character (like 4).
                // For those cases, if we see a shift code, send that
                // separately.
    ...
    else
    {
        ...
        if (p != -1)
        {
            //send the key
            ptermSendKey (p);
            // look ahead to see if line should be split at this
            // breakpoint (space or hyphen)
            if (autonext != 0 &&
                !ptermApp->m_splitWords && 
                (c == wxT (' ') || c == wxT ('-')))
            {
                for (tindex = nextindex, neednext = true,
                       tcnt = m_pasteNextKeyCnt;
                     tindex < m_pasteText.Len () && neednext &&
                              tcnt < autonext;
                     tindex++, tcnt++)
                {
                    tchr = m_pasteText[tindex];
                    if (tchr == wxT (' ') || tchr == wxT ('-')  ||
                        tindex == m_pasteText.Len () - 1)
                        neednext = false;
                }
                if (neednext)
                    m_pasteNextKeyCnt = 0;
    ...
    if (nextindex < m_pasteText.Len ())
    {
        ...
        if (neednext)
        {
            ...
            m_pasteNextKeyCnt = 0;
            ptermSendKey (026);
            for (; !ptermApp->m_splitWords &&
                      m_pasteText[nextindex] == wxT (' ') &&
                      nextindex < m_pasteText.Len ();
                  nextindex++); //eat leading spaces
            m_pasteIndex = nextindex;
            delay = atoi (ptermApp->m_lineDelay.mb_str ());
            m_pasteTimer.Start (delay, true);
            m_bPasteActive = true;
...
{
    int key[] = {0024, 0000, 0103, 0137, 0132, 0103, 0136, 0105, 0122, 0122,
                 0106, 0024, 0001, -1};
    ptermSendKeys (key);
}
void PtermFrame::OnMacro2 (wxCommandEvent &event)
{
    int key[] = {0024, 0000, 0103, 0137, 0132, 0103, 0136, 0111, 0116, 0106,
                 0117, 0024, 0001, -1};
    ptermSendKeys (key);
}
void PtermFrame::OnMacro3 (wxCommandEvent &event)
{
    int key[] = {0024, 0000, 0103, 0137, 0132, 0103, 0136, 0113, 0105, 0131,
                 0123, 0024, 0001, -1};
    ptermSendKeys (key);
}
void PtermFrame::OnMacro4 (wxCommandEvent &event)
{
    int key[] = {0024, 0000, 0103, 0137, 0132, 0103, 0136, 0124, 0105, 0130,
                 0124, 0024, 0001, -1};
    ptermSendKeys (key);
}
void PtermFrame::OnMacro5 (wxCommandEvent &event)
{
    int key[] = {0103, 0117, 0114, 0117, 0122, 0100, 0100, 0100, 0104, 0111,
                 0123, 0120, 0114, 0101, 0131, 0134, 0132, 0103, 0136, 0105,
                 0122, 0122, 0106, -1};
    ptermSendKeys (key);
}
...
{
    int key[] = {0103, 0117, 0114, 0117, 0122, 0100, 0100, 0100, 0104, 0111,
                 0123, 0120, 0114, 0101, 0131, 0134, 0132, 0103, 0136, 0111,
                 0116, 0106, 0117, -1};
    ptermSendKeys (key);
}
...
{
    int key[] = {0103, 0117, 0114, 0117, 0122, 0100, 0100, 0100, 0104, 0111,
                 0123, 0120, 0114, 0101, 0131, 0134, 0132, 0103, 0136, 0113,
                 0105, 0131, 0123, -1};
    ptermSendKeys (key);
}
...
{
    int key[] = {0103, 0117, 0114, 0117, 0122, 0100, 0100, 0100, 0104, 0111,
                 0123, 0120, 0114, 0101, 0131, 0134, 0132, 0103, 0136, 0124,
                 0105, 0130, 0124, -1};
    ptermSendKeys (key);
}
void PtermFrame::OnMacro9 (wxCommandEvent &event)
{
    ...
    ptermSendKeys (key0);
    int key3[] = {0024, 0000, 0103, 0137, 0132, 0103, 0136, 0113, 0105, 0131,
                  0123, 0024, 0001, -1};
    ptermSendKeys (key3);
    int key[] = {0034, 0034, -1};
    ptermSendKeys (key);
...
//
// It also enables or disables display of the status bar and menu bar,
// according to the current preferences.  Note that the objects for
// these two always exist, but they are or are not attached to the frame,
...
void PtermFrame::ptermBlockErase (int x1, int y1, int x2, int y2)
{
    ...
    for (int row = srow; row < srow + rows; row++)
    {
        ...
        {
            textmap[row * 64 + col][0] = textmap[row * 64 + col][1] = '\0';
        }
    }
...
void PtermFrame::SaveChar (int x, int y, wxChar c, bool large_p)
{
    ...
    // Save the Unicode character code
    textmap[y * 64 + x][0] = c;
    textmap[y * 64 + x][1] = '\0';
    
    // Todo: autobackspace -> combining diacritical code
    if (large_p)
    {
        x = (x + 1) & 077;
        textmap[y * 64 + x][0] = '\0';
...
//  wxButton* btnDefaults;
    ...
    lblExplainConversions = new wxStaticText (tab5, wxID_ANY,
                                              _("* NOTE: Conversions are applied at 8-character intervals."),
                                              wxDefaultPosition,
                                              wxDefaultSize, 0);
...
void PtermCanvas::OnDraw (wxDC &dc)
{
    ...
    if (rh != 0 && rw != 0)
    {
        ...
        debug ("Drawing selection region onto the window canvas, top %d %d, size %d %d",
